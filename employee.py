...
def employeeData(eid):
    ...
    def deleteEmployee():
        ...
        dbs.db.commit()
        messagebox.showinfo('Success','recoed has been removed!')
        empform.destroy()

    def updateEmployee():
        if name.get()=='' or dob.get()=='' or email.get()=='' or address.get()=='' or profile.get()=='' or salary.get()=='':
            ...
            email='"+contact.get()+"',address='"+address.get()+"' ,profile='"+profile.get()+"' ,\
            department='"+str(dpid)+"',salary='"+str(salary.get())+"'where id='"+str(eid)+"'")
    ...
    lbl=Label(empform,text='DOB',font='arial 11 bold')
    ...
    dropdown.grid(row=6,column=1,padx=5,pady=5)
...
def backup():
    getempdata=allemployee()
    fname='emp_backup'+str(datetime.datetime.now().strftime('%d_%m_%y_%I_%M_%S'))+'.xls'
    fobj=open(fname,'w')
    header='Sr\tName\tBirthdate\tContact\tEmail\tAddress\tProfile\tSalary\tDepartment\n'
    fobj.write(header)
    sr=1
    for val in getempdata:
        ...
        sr+=1
    fobj.close()
    messagebox.showwarning('success','backup file created')
def createemployee():
    def saveEmployee():
        if name.get()=='' or dob.get()=='' or email.get()=='' or address.get()=='' or profile.get()=='' or salary.get()=='':
            messagebox.showwarning('Warning','All field are mandatory')
    ...
    lbl=Label(regform,text='DOB',font='arial 11 bold')
